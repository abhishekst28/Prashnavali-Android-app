...
public final class zzedq {
    ...
    public static ECParameterSpec zza(zzeds zzeds) throws NoSuchAlgorithmException {
        int i = zzedp.zzhzc[zzeds.ordinal()];
        if (i == 1) {
        ...
        String valueOf = String.valueOf(zzeds);
    ...
    public static ECPublicKey zza(zzeds zzeds, byte[] bArr, byte[] bArr2) throws GeneralSecurityException {
        ECParameterSpec zza = zza(zzeds);
        ECPoint eCPoint = new ECPoint(new BigInteger(1, bArr), new BigInteger(1, bArr2));
        zza(eCPoint, zza.getCurve());
        return (ECPublicKey) zzedt.zzhzs.zzhs("EC").generatePublic(new ECPublicKeySpec(eCPoint, zza));
    }

    public static KeyPair zza(ECParameterSpec eCParameterSpec) throws GeneralSecurityException {
        KeyPairGenerator zzhs = zzedt.zzhzr.zzhs("EC");
        zzhs.initialize(eCParameterSpec);
        return zzhs.generateKeyPair();
    }
    ...
    public static byte[] zza(ECPrivateKey eCPrivateKey, ECPoint eCPoint) throws GeneralSecurityException {
        ...
        PublicKey generatePublic = zzedt.zzhzs.zzhs("EC").generatePublic(new ECPublicKeySpec(eCPoint, eCPrivateKey.getParams()));
        KeyAgreement zzhs = zzedt.zzhzq.zzhs("ECDH");
        zzhs.init(eCPrivateKey);
        try {
            zzhs.doPhase(generatePublic, true);
            byte[] generateSecret = zzhs.generateSecret();
